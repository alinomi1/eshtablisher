...
// Import the Embla Wheel Gestures Plugin
...
const Services = () => {
    ...
    useEffect(() => {
        ...
        // Listen to select (snap) and scroll (continuous drag/wheel) events
        api.on("select", updateCarousel);
        api.on("scroll", updateCarousel);

        return () => {
            api.off("select", updateCarousel);
            api.off("scroll", updateCarousel);
        };
    }, [api]);
        ...
        <section className="bg-[#F6F6F6] py-12 md:py-14 lg:py-16 overflow-hidden">
            <div className="container-1200 px-5">
                ...
                <div className='mt-8 sm:mt-10 w-full'>
                    <div className="flex items-center gap-4 sm:gap-6 lg:gap-10 w-full px-0 sm:px-1 md:px-2">
                        {/* Visual Progress Bar */}
                        <div className="h-2 grow rounded-[10px] bg-white relative min-w-full overflow-hidden">
                            <div
                                className="absolute left-0 top-0 h-full bg-[#E5C999] rounded-[10px] transition-all duration-300 ease-out"
                                style={{ width: `${progress}%` }}
                            ></div>
                        </div>
